Shader "Custom/UIShader"
{
	...
	_Color("Tint", Color) = (1,1,1,1)
		...
		_DistortionDamper ("Distortion Damper", Float) = 3
	...
	fixed4 frag(v2f IN) : SV_Target
	{
		float2 offset = float2(
			tex2D(_NoiseTex, float2(IN.worldPosition.y / _DistortionSpreader, _Time[1])).r,
			tex2D(_NoiseTex, float2(_Time[1], IN.worldPosition.x / _DistortionSpreader)).r
		);
